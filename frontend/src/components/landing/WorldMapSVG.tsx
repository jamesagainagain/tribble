...
export const WorldMapSVG = () => {
  ...
  const projection = useMemo(
    () => d3Geo.geoNaturalEarth1().fitSize([WIDTH, HEIGHT], { type: 'Sphere' } as any),
    []
  );
  ...
  return (
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full max-w-6xl" fill="none">
      {/* Graticule grid */}
      <motion.path
      ...
      {worldData.features.map((feature, i) => {
        ...
        return (
          <motion.path
            ...
            transition={{ duration: 2.5, delay: 0.5 + (i % 20) * 0.1, ease: 'easeInOut' }}
          />
        );
      })}
