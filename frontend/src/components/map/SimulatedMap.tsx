...
import { PLACEHOLDER_EVENTS, PLACEHOLDER_DRONES, PLACEHOLDER_ZONES, PLACEHOLDER_BOUNDARIES, PLACEHOLDER_NGOS } from '@/lib/placeholder-data';
import { CONFLICT_ZONES, ROUTE_COLORS, ROUTE_DASH } from '@/lib/conflict-zones';
import { WORLD_CITIES, COUNTRY_CENTROIDS } from '@/lib/cities-data';
import { ONTOLOGY_ICONS, ONTOLOGY_TO_LAYER } from '@/lib/icon-registry';
import { ZONE_FILL_STYLES } from '@/lib/zone-patterns';
...
// Dark style spec — no external tiles needed, uses free OSM/Protomaps
...
function buildRoutesGeoJSON(zoneId: string | null): GeoJSON.FeatureCollection {
  if (!zoneId) return { type: 'FeatureCollection', features: [] };
...
function buildNGOZonesGeoJSON(): GeoJSON.FeatureCollection {
  return {
    type: 'FeatureCollection',
    features: PLACEHOLDER_NGOS.map(n => ({
      type: 'Feature' as const,
      properties: {
        id: n.id,
        name: n.abbreviation,
...
export const SimulatedMap = () => {
  ...
  const { setSelectedEventId, setRightPanelOpen, setRightPanelTab, activeConflictZoneId } = useUIStore();
  const { visibility, opacity } = useLayerStore();
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addSource('boundaries', { type: 'geojson', data: buildBoundariesGeoJSON() });
      map.addSource('routes', { type: 'geojson', data: buildRoutesGeoJSON(null) });
      map.addSource('ngo-zones', { type: 'geojson', data: buildNGOZonesGeoJSON() });
      ...
      map.addLayer({
        ...
        paint: {
          'fill-color': [
            ...
            'controlled_territory', 'rgba(123, 97, 255, 0.06)',
            'displacement_corridor', 'rgba(123, 97, 255, 0.06)',
            'transparent',
          ],
          'fill-opacity': 0.8,
        },
      ...
      map.addLayer({
        ...
        paint: {
          'line-color': [
            ...
            'safe_zone', '#00FF88',
            'controlled_territory', '#7B61FF',
            'displacement_corridor', '#7B61FF',
            '#1E2D4A',
          ],
          'line-width': 1.5,
          'line-dasharray': [4, 3],
      ...
      map.addLayer({
        ...
        paint: {
          'line-color': [
            ...
            'ceasefire_line', '#7B61FF',
            'frontline_active', '#FF2D55',
            'administrative_boundary', '#8892A4',
            '#1E2D4A',
          ],
          'line-width': [
            'match', ['get', 'boundary_type'],
          ...
          'line-opacity': [
            'match', ['get', 'boundary_type'],
            'international_border', 0.4,
            'administrative_boundary', 0.3,
            0.8,
          ],
          'line-dasharray': [4, 3],
        },
      ...
      map.addLayer({
        ...
        paint: {
          ...
          'heatmap-color': [
            ...
            0, 'rgba(0, 212, 255, 0)',
            0.2, 'rgba(0, 212, 255, 0.15)',
            0.5, 'rgba(255, 149, 0, 0.3)',
            0.8, 'rgba(255, 45, 85, 0.5)',
            1, 'rgba(255, 45, 85, 0.7)',
          ],
          'heatmap-radius': 30,
          'heatmap-opacity': [
      ...
      map.addLayer({
        ...
        paint: {
          ...
          'circle-radius': [
            'interpolate', ['linear'], ['zoom'],
            2, ['match', ['get', 'severity'], 'critical', 5, 'high', 4, 3],
            6, ['match', ['get', 'severity'], 'critical', 8, 'high', 6, 5],
            12, ['match', ['get', 'severity'], 'critical', 12, 'high', 10, 8],
          ],
      ...
      map.on('mouseenter', 'events-circles', (e) => {
        ...
        if (feature) {
          ...
          const evt = PLACEHOLDER_EVENTS.find(ev => ev.id === feature.properties?.id);
          if (evt) {
            // Convert HipEvent to Incident for tooltip
            const incident: Incident = {
              id: evt.id,
              type: evt.ontology_class as any,
              ...
              verification_status: evt.verification_status as any,
  ...
  // Listen for flyTo events from conflict ticker
  useEffect(() => {
  ...
  // Update routes when active conflict zone changes
  useEffect(() => {
  ...
  // Sync layer visibility with Zustand store
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !map.isStyleLoaded()) return;

    // Map layer IDs to our visibility keys
    const layerMapping: Record<string, string[]> = {
      'zones-fill': ['B3_safe_zones', 'B4_no_go_zones', 'A5_controlled_territory'],
      'zones-outline': ['B3_safe_zones', 'B4_no_go_zones', 'A5_controlled_territory'],
      'zones-labels': ['B3_safe_zones', 'B4_no_go_zones'],
      'boundaries-line': ['A1_intl_borders', 'A2_disputed_borders', 'A4_frontlines'],
      'events-heat': ['D1_risk_heatmap'],
      'events-circles': ['C1_armed_conflict', 'C2_infrastructure', 'C3_displacement', 'C4_aid_humanitarian', 'C5_natural_environmental'],
    ...
    for (const [mlLayerId, vizKeys] of Object.entries(layerMapping)) {
      ...
      const anyVisible = vizKeys.some(k => visibility[k as keyof typeof visibility]);
      map.setLayoutProperty(mlLayerId, 'visibility', anyVisible ? 'visible' : 'none');
    }
  }, [visibility]);
