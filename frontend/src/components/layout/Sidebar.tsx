...
import {
  Map, AlertTriangle, Brain, Navigation, FileText, Settings, LogOut, ChevronLeft, Inbox,
  Send, Route, Bell, Shield, Users, Activity, BarChart3
} from 'lucide-react';
...
const ICON_MAP = {
  Map, AlertTriangle, Brain, Navigation, FileText, Settings, Inbox,
  Send, Route, Bell, Shield, Users, Activity, BarChart3,
} as const;
...
export const AppSidebar = () => {
  ...
  const criticalCount = PLACEHOLDER_EVENTS.filter(e => e.severity === 'critical' && e.verification_status !== 'verified').length;
  const pendingCount = PLACEHOLDER_SUBMISSIONS.filter(s => s.status === 'pending' || s.status === 'in_review').length;
  ...
  return (
    ...
    >
      ...
      >
        {sidebarExpanded ? (
          <div className="flex items-center justify-between w-full">
            <div>
              <p className="font-mono text-[11px] text-primary tracking-widest leading-none">HIP</p>
          ...
          <div className="flex items-center justify-center w-full">
            <span className="font-mono text-sm text-primary tracking-widest">H</span>
          </div>
        )}
      </div>

      {/* Role indicator (expanded) */}
      <AnimatePresence>
        {sidebarExpanded && (
          <motion.div
          ...
          >
            <div className="flex items-center gap-1.5">
              <div className={`w-1.5 h-1.5 rounded-full ${
                activeRole === 'admin' ? 'bg-destructive' : activeRole === 'organization' ? 'bg-primary' : 'bg-[hsl(var(--hip-green))]'
              }`} />
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.path}
              className={`
                group flex items-center gap-3 h-9 px-2 rounded-sm transition-all relative
              ...
              <span className="relative flex-shrink-0">
                ...
                {badge > 0 && (
                  <motion.span
                    ...
                    animate={item.badgeKey === 'events' ? { scale: [1, 1.15, 1] } : undefined}
                    transition={item.badgeKey === 'events' ? { duration: 2, repeat: Infinity } : undefined}
                  >
                    {badge}
                  </motion.span>
                )}
      ...
      {/* NGO Zone selector (expanded only, org/admin only) */}
      <AnimatePresence>
        {sidebarExpanded && activeRole !== 'civilian' && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <p className="font-heading text-[9px] tracking-wider text-muted-foreground mb-2 uppercase">NGO Zone</p>
            <select className="w-full h-7 rounded-sm border border-border bg-card px-2 text-[11px] font-mono text-foreground">
              ...
              {PLACEHOLDER_NGOS.map((ngo) => (
                <option key={ngo.id} value={ngo.id}>{ngo.abbreviation} — {ngo.zone_name}</option>
              ))}
            </select>
          </motion.div>
        )}
      ...
      <div className="relative border-t border-border p-2">
        {sidebarExpanded ? (
          <>
            ...
            >
              <div className="w-7 h-7 rounded-full bg-primary/20 border border-primary/40 flex items-center justify-center flex-shrink-0">
                <span className="font-mono text-[10px] text-primary">{user?.avatar_initials || 'SC'}</span>
              </div>
              <div className="text-left overflow-hidden">
                <p className="font-body text-xs text-foreground leading-none whitespace-nowrap">{user?.name || 'Sarah Chen'}</p>
                <p className="font-mono text-[9px] text-muted-foreground mt-0.5 whitespace-nowrap uppercase">{ROLE_LABELS[activeRole]}</p>
              </div>
            </button>
            ...
            <AnimatePresence>
              {showUserPopover && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 8 }}
                >
                  <p className="font-body text-sm text-foreground">{user?.name || 'Sarah Chen'}</p>
                  <p className="font-mono text-[10px] text-muted-foreground">{user?.email || 'sarah.chen@relief.io'}</p>
                  ...
                  >
                    ...
                    <span className="font-heading text-xs tracking-wider">SIGN OUT</span>
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          /* Collapsed: just sign out on click */
          <button
